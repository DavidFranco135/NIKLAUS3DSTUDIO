...
| **PrusaSlicer** (CLI) | AGPL-3.0 | Sim, como binário externo via subprocess/CLI | **Stub (Fase 10)** — `PrusaSlicerCLIProvider` implementa a interface real de `SlicerProvider` mas levanta `ProviderNotConfiguredError`; binário **não foi baixado, instalado nem executado** nesta máquina | AGPL tem cláusula de "uso via rede" — como é executado como ferramenta local pelo worker (não é modificado e redistribuído como serviço de terceiro), risco é baixo, mas **jurídico deve confirmar** antes de produção, especialmente se a plataforma for oferecida como SaaS público. Decisão explícita na Fase 10: não instalar/baixar o binário até essa confirmação existir |
| **OrcaSlicer** (CLI) | AGPL-3.0 | Mesma observação do PrusaSlicer | **Stub (Fase 10)** — mesma situação do PrusaSlicer, `OrcaSlicerCLIProvider` | Idem |
