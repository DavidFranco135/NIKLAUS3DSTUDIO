...
**Status na Fase 12 (Estoque):** implementada — `materials`, `inventory_items` e `inventory_movements` (DATABASE.md), com `domain/inventory/` (puro, sem I/O): `movement.py` valida o tipo do movimento e exige que o sinal de `quantity` combine com ele (`entrada` > 0; `saida`/`consumo`/`perda` < 0; `ajuste` pode ser qualquer um, exceto zero) e `apply_movement` nunca deixa o estoque ficar negativo — inclusive para `ajuste`, decisão explícita (documentada no docstring): estoque negativo é sinal de número errado, não de encomenda em atraso (isso é outro módulo). `alerts.py::is_low_stock` é reaproveitado tanto para o campo `is_low_stock` de cada item na resposta da API quanto para o filtro `?low_stock_only=true` de listagem — comparação feita em Python sobre os itens já carregados, não duplicada como filtro SQL, para não ter dois lugares definindo "estoque baixo". Endpoints em `interfaces/http/v1/materials.py` e `inventory.py`: `POST/GET /organizations/{id}/materials[/{id}]`, `POST/GET /organizations/{id}/inventory-items[/{id}]` e `POST/GET .../inventory-items/{id}/movements` (papel mínimo `MANAGER` para cadastrar material/item, `OPERATOR` para lançar movimento, `VIEWER` para ler). `inventory_movements.reference_order_id` é coluna solta sem FK ainda — `orders` só existe na Fase 14. Catálogo de impressoras/máquinas (`machines`, usado por `machine_cost_per_hour` na Calculadora) **não** faz parte desta fase — é a Fase 17.
...
"Histórico" (do título da fase, "Clientes: CRUD + histórico") por ora é `GET /organizations/{id}/customers/{id}/history`, retornando os `projects` e `quotes` vinculados ao cliente — ainda não incluía pedidos/entregas porque `orders` só existia a partir da Fase 14 (nota abaixo); quando essa fase implementou `orders`, o endpoint ganhou uma chave `orders` nova na resposta — mudança de formato aditiva (chave nova, nada removido), não a "sem mudar de formato" que esta nota originalmente previa.
...
**Status na Fase 14 (Pedidos):** implementada — tabelas `orders` e `order_items` (DATABASE.md), com `domain/orders/` (puro, sem I/O):
- `status.py::validate_transition` — máquina de estados do funil `quote → order → paid → production → printing → finishing → packaging → delivered → completed`, mais o terminal lateral `cancelled`. Decisão explícita: só permite **avançar** (nunca voltar), mas **não exige passo a passo** — pular um estágio que não se aplica a um pedido específico é permitido (ex.: `quote → paid` direto). `cancelled` é alcançável de qualquer estágio não-terminal, nunca a partir de `completed` nem de `cancelled` (já são terminais).
- `totals.py::compute_total_amount` — soma `quantity * unit_price` de todos os `order_items` de um pedido; tratado como `0` quando `unit_price` é `None`. Recalculado do zero a cada item adicionado (não incrementado), evitando drift de arredondamento acumulado.
- `create_order` aceita um `quote_id` opcional: quando informado, o pedido nasce com `total_amount` igual a `quote.suggested_price` (ou `quote.final_price`, se negociado) e marca a cotação como `accepted` — o único lugar do sistema que transiciona `quotes.status` até agora (`sent`/`rejected`/`expired` não têm endpoint ainda, fora do escopo desta fase). Sem `quote_id`, o pedido nasce com `total_amount=0` e ganha valor conforme itens são adicionados.
- Fecha a última coluna solta que vinha sendo adiada desde a Fase 12: `inventory_movements.reference_order_id` ganhou a `FOREIGN KEY` para `orders` (mesma migration desta fase); `create_movement` agora aceita `reference_order_id` e valida que o pedido existe na organização antes de vincular.
- `order_items.machine_id` é a nova coluna solta, sem FK — `machines` só existe na Fase 17, mesmo padrão adiado já usado para `quotes.customer_id`/`inventory_movements.reference_order_id`/`projects.customer_id` em fases anteriores.
...
**Status na Fase 18A (SaaS/Billing — infraestrutura interna, sem pagamento real):** implementada exatamente no escopo combinado — só a infraestrutura de planos/assinatura/entitlements/uso/limites; **nenhuma conta Stripe, credencial real, cobrança ou checkout foi criado ou sequer esboçado.**

- Tabelas `plans`, `plan_entitlements`, `subscriptions`, `billing_events` (DATABASE.md). `plan_entitlements` é genérica (chave→valor, `limit_type` boolean/numeric/unlimited) de propósito — cada novo limite ou feature flag futuro é uma linha nova, nunca uma migration.
- `domain/billing/` (puro, sem I/O): `subscription_status.py::validate_transition` — máquina de estados `trialing → active ⇄ past_due → canceled` (vocabulário emprestado do Stripe de propósito, para não remapear nada quando um provider real existir); `entitlements.py` — `Entitlement`/`is_feature_enabled`/`get_numeric_limit`; `enforcement.py` — `check_numeric_limit`/`check_feature_flag`, nunca importa repositório de outro módulo (ver abaixo).
- **Padrão de integração deliberado:** o módulo de billing nunca sabe contar projeto, job de IA ou byte de arquivo — quem sabe contar seu próprio recurso é o módulo dono dele. `create_project` (Fase 3), `create_ai_job` (Fase 4) e `confirm_upload` (Fase 3) contam a si mesmos e chamam `application/billing/use_cases.py::enforce_numeric_limit(key, current_usage)`, que busca o entitlement do plano atual e compara. Isso evita billing importar `ProjectRepository`/`AIJobRepository`/etc. — zero dependência circular entre módulos de negócio e billing (a única exceção é `get_usage_summary`, que **é** uma leitura de exibição, não enforcement, e por isso pode compor os outros repositórios diretamente, mesmo padrão já usado pelo Dashboard da Fase 16).
- **Limite atingido → `402 Payment Required`**, com corpo estruturado (`{"message", "limit_key", "current_usage", "limit"}`, não só uma string) — decisão confirmada com o usuário antes de implementar. `LimitExceededError` carrega esses campos como atributos; `interfaces/http/errors.py::as_http_exception` monta o corpo especial só para essa exceção.
- Toda organização nasce com uma assinatura — `create_default_subscription` é chamado a partir de `register()` e `create_organization()` (nunca fica "sem plano", o que deixaria o enforcement indefinido). Idempotente (`UNIQUE(organization_id)` em `subscriptions`, mesmo espírito de `AIJob.idempotency_key`). O plano semeado, `dev_unlimited` (20 projetos, 50 jobs de IA/período, 500MB, features de IA generativa ligadas), **não é um plano comercial** — é fixture de desenvolvimento; preços e planos reais são decisão de produto futura, explicitamente fora do escopo desta fase. Semeado tanto por uma migration de dados (`a64f7589aaf0`) quanto, de forma idempotente, por `get_default_plan()` na primeira chamada — necessário porque ambientes que criam o schema via `Base.metadata.create_all` (notavelmente os testes automatizados) nunca rodam migrations.
- `domain/billing/ports.py::BillingProvider` — protocolo (`name`+`health_check`, mesmo padrão de todo outro provider da plataforma) com `create_subscription`/`cancel_subscription`/`change_plan`/`get_subscription`/`verify_webhook_signature`/`parse_webhook_event`. `MockBillingProvider` (`infrastructure/billing_providers/mock.py`) implementa tudo em processo, sem rede — gera ids sintéticos, calcula período/trial a partir de `now()`. **Nenhum `StripeBillingProvider`, nem stub, foi criado** — decisão explícita para não ter nenhum código com o nome "Stripe" no repositório antes da Fase 18B.
- Webhook: `POST /api/v1/billing/webhooks/{provider_name}` — fora de `/organizations/{id}/...` de propósito (a organização é resolvida a partir do payload do evento, não da URL/JWT — o mesmo modelo do Stripe real). Idempotência via `UNIQUE(provider, external_event_id)` em `billing_events`: uma reentrega do mesmo evento é detectada e devolve 200 sem reprocessar. Processamento síncrono por ora (é leve) — diferente do "tudo pesado é assíncrono" da Fase 4, mas revisável se algum dia deixar de ser leve.
- RBAC: mudar de plano ou cancelar assinatura exige **`OWNER`** — mais restrito que o `MANAGER` que já opera o financeiro interno (Fase 15), decisão confirmada com o usuário (é o pagamento da própria organização à plataforma, não a contabilidade interna dela). Qualquer papel (`VIEWER`+) lê plano/uso/limites.
- Frontend: `apps/web/app/billing/page.tsx` — painel só-leitura (plano, status, período, trial) + tabela de uso com contagem `atual/limite` por chave + duas ações (cancelar agora / cancelar ao fim do período / trocar de plano), visíveis só para quem é `OWNER` na organização atual. Link "Faturamento" adicionado ao `AppHeader`. `lib/api-client.ts::extractDetailMessage` ganhou um caso a mais para extrair `.message` do corpo estruturado do `LimitExceededError` (sem isso, o erro apareceria como "Payment Required" genérico em vez da mensagem real).
- **Pendência sinalizada, não resolvida:** `Organization.plan` (coluna de texto livre, default `"free"`, existe desde a Fase 1) ficou **órfã** — nunca foi atualizada em lugar nenhum e agora está superada pelo par `Subscription`→`Plan` real construído nesta fase. Não foi removida agora para não misturar uma mudança de schema não relacionada (e um ajuste de `OrganizationResponse`/frontend) dentro desta fase; é candidata a limpeza numa fase futura.
- **Estratégia para ativar o Stripe depois (Fase 18B):** `StripeBillingProvider` implementa o mesmo `BillingProvider` e substitui `MockBillingProvider` no registry — zero mudança em domain/application/HTTP. `subscriptions.external_*` já existem (nulos, não usados pelo mock hoje) — migrar uma assinatura mock para uma real é um `UPDATE`, não uma migration. O path do webhook já aceita `{provider_name}` — acrescentar `"stripe"` não muda rota nenhuma. Checkout real, preços e cobrança de verdade continuam explicitamente fora do escopo até essa fase existir, sob aprovação explícita do usuário.
