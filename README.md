...
> **Status atual: FASE 11 — Calculadora de custos (motor real, 100% determinístico).** `domain/calculator/engine.py::calculate_quote` é uma função pura que transforma `QuoteInputs` (material, tempo de impressão, custo de máquina/hora, energia, mão de obra — hoje informados manualmente, já que o Slicer da Fase 10 ainda é só stub) e `CostProfileValues` (os fatores configuráveis por organização: energia, mão de obra, embalagem, desperdício, taxas, margem, imposto) num `CostBreakdown` completo. Nenhuma IA participa da conta — só pode alimentar dados de entrada. Novas tabelas `cost_profiles` e `quotes` (múltiplos perfis por organização; cada orçamento grava um snapshot imutável do cálculo, então mudar o perfil depois não altera orçamentos antigos — testado). Endpoints REST em `/organizations/{id}/cost-profiles` e `/organizations/{id}/quotes`. CAD paramétrico (Fase 7), Mesh Processing (Fase 8) e Printability (Fase 9) continuam reais; Slicer (Fase 10) é abstração+stub; Text-to-3D e Image-to-3D continuam mock. Ver [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md) §14 e [docs/DATABASE.md](docs/DATABASE.md).
>
