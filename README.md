...
> **Status atual: FASE 5 — Text-to-3D (interfaces prontas, sem modelo real).** Decisão explícita: nenhuma API paga de terceiros integrada, nenhum modelo baixado/rodado localmente (esta máquina não tem GPU confirmada). Em vez disso, a superfície completa de providers está pronta: `ImageTo3DProvider`, `MeshRepairProvider` e `TextureProvider` ganharam mocks funcionais (geometria/textura em Python puro), e Hunyuan3D/TRELLIS/Stable Fast 3D/SPAR3D têm *stubs* que implementam a interface real e avisam claramente que dependem de infra de GPU + confirmação de licença (`ProviderNotConfiguredError`). Trocar um stub por um adapter funcional não deve exigir tocar no AI Orchestrator nem no registry.
>
