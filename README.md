...
> **Status atual: FASE 17 — Integração com impressoras (perfis).** Só cadastro de máquinas — nenhuma conectividade real (isso é trabalho futuro, como o próprio roadmap já dizia). Nova tabela `machines` (nome, marca/modelo, tecnologia FDM/SLA/MSLA, volume, potência, `cost_per_hour`, status active/maintenance/inactive — sem soft delete). CRUD em `/organizations/{id}/machines` (`MANAGER` para cadastrar/editar, `VIEWER` para ler). Fecha `order_items.machine_id`, que ficava sem FK desde a Fase 14. **Integração com a Calculadora:** `POST /quotes` ganhou um `machine_id` opcional que busca `cost_per_hour` do perfil da máquina em vez de exigir o valor manual — mudança aditiva e retrocompatível (o caminho manual continua funcionando, testado lado a lado com o novo). Dashboard (Fase 16), Financeiro (Fase 15), Pedidos (Fase 14), Clientes (Fase 13), Estoque (Fase 12), Calculadora (Fase 11), CAD paramétrico (Fase 7), Mesh Processing (Fase 8) e Printability (Fase 9) continuam reais; Slicer (Fase 10) é abstração+stub; Text-to-3D e Image-to-3D continuam mock. Ver [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md) (nota "Status na Fase 17").
>
